...
void CDibView::OnDraw(CDC* pDC)
{
	...
	if (hDIB != NULL)
	{
		...
		pDC->MoveTo(rcDest.left,rcDest.top+10);
		pDC->LineTo(rcDest.right,rcDest.top+10);
		pDC->MoveTo(rcDest.left+10,rcDest.top);
		pDC->LineTo(rcDest.left+10,rcDest.bottom);
		pDC->SelectObject(oldpen);
		CRect rect;
		pDC->GetClipBox(&rect);
		::PaintDIB(pDC->m_hDC,&rect,pDoc->GetHDIB(),&rect,pDoc->GetDocPalette());
		pDC->SetBkMode(TRANSPARENT);
		pDC->TextOut(rect.left+105,rect.top+50,strLocation);
	}
}
...
void CDibView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	CRgn rgn;
	CRgn prergn;
	prergn.CreateRectRgn(m_Prepoint.x-100,m_Prepoint.y-50,m_Prepoint.x+100,m_Prepoint.y+50);
	rgn.CreateRectRgn(point.x-5,point.y-5,point.x+80,point.y+20);
	rgn.CombineRgn(&prergn,&rgn,RGN_OR);
	SelectClipRgn(GetDC()->m_hDC,(HRGN)rgn.GetSafeHandle());
	strLocation.Format("(%d,%d)",point.x,point.y);
...
void CDibView::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if(m_count<100)
		GetDocument()->m_Point[m_count++]=point;
