...
public:
	virtual BOOL InitInstance() {
		...
		//remove the test, because this app appears to run
		//on Windows XP SP2 as well
/*		if(!Is_Win2000()){
			::AfxMessageBox("Must run on Windows 2000");
			return FALSE;
		}*/
		//end add by jiangsheng 2002-11-3

		//begin add by jiangsheng 2006-4-17

		//Test if the app is running in a remote location such as mapped drive
		//for information can be fount at 
        ...
        DWORD               dwRet = WNetGetUniversalName(CTaskKeyMgr::strRemoteDllName,
			REMOTE_NAME_INFO_LEVEL, uncName,
            &dwSize);

		if(dwRet!=ERROR_NOT_CONNECTED||PathIsUNC(CTaskKeyMgr::strRemoteDllName))
		{
			::AfxMessageBox("Remote launch detected, This program can not run remotely.");
			return FALSE;
		}

		//
		//end add by jiangsheng 2006-4-17
		CMyDialog dlg;
		m_pMainWnd = &dlg;
		dlg.DoModal();
		return FALSE;
