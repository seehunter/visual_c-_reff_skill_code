...
void COpenFileDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR COpenFileDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}

void COpenFileDlg::OnBnClickedOpenfile()
{
	// TODO: 在此添加控件通知处理程序代码
	static char BASED_CODE szFilter[] = "圣德文件 (*.dmt)|*.dmt|All Files (*.*)|*.*||";
	CFileDialog dlg( FALSE, NULL, NULL, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, szFilter);
	if( IDOK != dlg.DoModal()){
		return;
	}
	CString ting;
	ting = dlg.GetPathName();

	CWnd* pWnd = NULL;

	pWnd = GetDlgItem(IDC_FILE_PATH);
